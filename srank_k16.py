...
parser = argparse.ArgumentParser(description='This script extract 21 embedding matrices from dlrm_k.m and linp_16_k.m')
...
sd_16_k = torch.load('../linp_16_'+str(K)+'_normal.m')['state_dict']
...
for name, value in sd_16_k.items():
    ...
    if header == 'emb_l':
        ...
        n, d = list(value.size())
        assert d == 16, str(name)+': inner dimension doesn\'t match, specified 16 but get %s'%(d)
...
pickle.dump((layer1_efranks,layer2_efranks),open('data/srank_inner_16_dimension_'+str(K)+'_tables.pkl','wb'))
