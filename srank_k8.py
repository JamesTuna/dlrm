...
parser = argparse.ArgumentParser(description='This script extract 21 embedding matrices from dlrm_k.m and linp_8_k.m')
...
sd_8_k = torch.load('../linp_8_'+str(K)+'_normal.m')['state_dict']
...
for name, value in sd_8_k.items():
    ...
    if header == 'emb_l':
        ...
        n, d = list(value.size())
        assert d == 8, str(name)+': inner dimension doesn\'t match, specified 8 but get %s'%(d)
...
pickle.dump((layer1_efranks,layer2_efranks),open('data/srank_inner_8_dimension_'+str(K)+'_tables.pkl','wb'))
